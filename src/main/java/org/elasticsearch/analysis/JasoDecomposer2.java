		...
		for(int i = 0; i < termBuffer.length; i++) {
			...
			if(ch >= 0xAC00 && ch <= 0xD7A3) {
				...
				for(int j=0; j < chosungKor[cho].length(); j++) {						
					buffer.append(chosungKor[cho].toCharArray()[j]);	
				}
				
				for(int j=0; j < jungsungKor[jung].length(); j++) {
					buffer.append(jungsungKor[jung].toCharArray()[j]);
				}
				
				//받침이 있으면
				if(jong != 0) {
					for(int j=0; j < jongsungKor[jong].length(); j++) {
						buffer.append(jongsungKor[jong].toCharArray()[j]);
					}					
				}
			} else {
				//한글을 제외한 문자는 그냥 넣는다.
				buffer.append(termBuffer[i]);
			}			
		}
